...
const AppointmentItem = ({ item }) => {
  ...
  const handleViewCustomer = (id) => {
    navigation.navigate("ViewCustomerById", { id });
  };
  ...
  return (
    ...
    >
      ...
      <View className={`flex flex-row justify-end items-center my-2`}>
        <TouchableOpacity
          onPress={() => handleViewCustomer(item?.appointment?.customer?._id)}
          className={`font-base text-xl bg-primary-accent rounded-lg px-5 py-3`}
        >
          <Text
            style={{
              color: invertTextColor,
